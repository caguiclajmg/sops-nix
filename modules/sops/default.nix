    ...
    (mkIf (cfg.secrets != {}) {
      ...
      system.activationScripts = {
        setupSecretsForUsers = mkIf (secretsForUsers != {}) (stringAfter ([ "specialfs" ] ++ optional cfg.age.generateKey "generate-age-key") ''
          ...
          ${withEnvironment "${sops-install-secrets}/bin/sops-install-secrets -ignore-passwd -use-ramfs=${boolToString cfg.useRamfs} ${manifestForUsers}"}
        ...
        setupSecrets = mkIf (regularSecrets != {}) (stringAfter ([ "specialfs" "users" "groups" ] ++ optional cfg.age.generateKey "generate-age-key") ''
          ...
          ${withEnvironment "${sops-install-secrets}/bin/sops-install-secrets -use-ramfs=${boolToString cfg.useRamfs} ${manifest}"}
        '' // lib.optionalAttrs (config.system ? dryActivationScript) {
          supportsDryActivation = true;
        });
